...
test('homepage editing and filter dismissal leave the workspace interactive',async()=>{
  ...
  try{
    ...
    await page.keyboard.press(mod+'+s');await expect(page.locator('.statusbar')).toContainText('已保存到本地')
    ...
    await fs.writeFile(path.join(project,'content/index.md'),saved+'\nExternal home edit.');await app.evaluate(({BrowserWindow},file)=>BrowserWindow.getAllWindows()[0].webContents.send('project:change',{kind:'content',file}),'index.md');await expect(preview.locator('article')).toContainText('External home edit.')
  ...
  }finally{const closed=app.waitForEvent('close');await app.evaluate(({app})=>app.exit());await closed;await fs.rm(root,{recursive:true,force:true,maxRetries:10,retryDelay:200})}
})
